...
class c_HIVAE_GridSearch:
    """
    Perform hyperparameter optimization on HIVAE using the grid search method. 
    """
    ...
    def __init__(self):
        ...
        self.max_epochs = int(cp.get('HyperparameterOptimization', 'gs_max_epochs'))
        self.final_n_epoch = 0
        self.cross_val_k_fold = int(cp.get('HyperparameterOptimization', 'gs_cross_val_k_fold'))
        self.early_stop_patience = int(cp.get('HyperparameterOptimization', 'gs_early_stop_patience'))
        self.seed = int(cp.get('HyperparameterOptimization', 'gs_seed_n'))
        self.search_options = {
            'ydims': [int(i) for i in cp.get('HyperparameterOptimization', 'gs_y_dimensions').split(',')],
            'nbatch': [int(i) for i in cp.get('HyperparameterOptimization', 'gs_batch_size').split(',')],
            'lrates': [float(i) for i in cp.get('HyperparameterOptimization', 'gs_learning_rates').split(',')],
            'wdecay': [float(i) for i in cp.get('HyperparameterOptimization', 'gs_weight_decay').split(',')],
            'beta1': [float(i) for i in cp.get('HyperparameterOptimization', 'gs_adam_beta_1').split(',')],
            'beta2': [float(i) for i in cp.get('HyperparameterOptimization', 'gs_adam_beta_2').split(',')]
        }
    ...
    def hyperopt_HIVAE(self):
        ...
        # create a dictionary containing all possible hyperopt search options
        ...
        for f in files:
            ...
            for opt in experiments:
                ...
                l_exp.append(self.run_network(settings, name, n_splits=self.cross_val_k_fold))
            l_files.append(l_exp)
            l_epochs.append(self.final_n_epoch)
        
        # list of grid search losses per variable group
        losses = list(zip(files, l_files)) 

        # list of indices of the minimum loss per variable group
            ...
            for epoch in range(args.epochs):
                ...
                if len(train_loss_epoch) >= 2:
                    if (train_loss_epoch[-1] >= train_loss_epoch[-2]):
                        ...
                        #print(f"[*] Train loss has not improved ({train_loss_epoch[-1]}). Early_stop counter = {no_improvement_counter}")
                    else: 
                        no_improvement_counter = 0
                        #print(f"[*] Train loss has improved ({train_loss_epoch[-1]}). Reset the early_stop counter.")
                ...
                if no_improvement_counter >= self.early_stop_patience:
                    print(f"[*] Train loss has not improved ({train_loss_epoch[-1]}) for {no_improvement_counter} times. Early stopping at epoch {epoch+1}/{args.epochs}.")
                    break
